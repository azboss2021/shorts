  ...
  return (
    <div className="flex flex-col gap-4">
      {questions.map((question, index) => (
        <div key={index} className="rounded-xl bg-purple-500/30">
          <button
            onClick={() => handleSetQuestion(index)}
            className="flex w-full cursor-pointer items-start justify-between gap-4 p-4 text-left text-lg font-bold"
          >
            {question.question}
            <FaAngleDown
              className={`mt-1.5 transition duration-300 ${currentQuestion === index ? "rotate-180" : ""}`}
            />
          </button>

          <div
            className={`grid overflow-hidden px-4 text-white/80 transition-all duration-300 ${index === currentQuestion ? "grid-rows-[1fr] pb-4" : "grid-rows-[0fr]"}`}
          >
